...
export default class GraphSet {
    ...
    public graphNames: Map<string, GraphConnection | undefined>;

    constructor(model: AppModel) {
        this.model = model;
        this.graphNames = new  Map<string, GraphConnection | undefined>();
    }
    ...
    getGraphConnectionWithName(name: string): GraphConnection | undefined {
    ...
    loadGraphNames(): Promise<any> {
        return new Promise<any>((resolve, reject) => {
            ...
            fs.ensureDir(path.resolve(this.model.userDataPath), 0o755, (err: any) => {
                if (err) {
                    reject(err);
                } else {
                        ...
                        if (err) {
                            ...
                            files.forEach((file: string) => {
                                let filename: string = path.basename(file, '.json');
                                    ...
                                    this.graphNames.set(filename, undefined);
